...
public class OrderManager implements Serializable {
	...
	// protected Time time = new Time(date.getHours(), date.getMinutes(),
	// date.getSeconds());
	...
	public void deliveryCycle() {
		for (int i = 0; i < Orderat.length; i++) {
			if (Orderat[i] == null)
				break;
			if (Orderat[i].getDeliverDay() >= LocalDate.now().getDayOfMonth()
					&& Orderat[i].getDeliverHour() >= date.getHours()) {
